...
void main() {
    ...
    float elevation = sin(modelPosition.x * uFrequencies.x + uTime * uSpeed) *
        sin(modelPosition.z * uFrequencies.y + uTime * uSpeed) * uElevation;

    for(float i = 1.0; i <= uChaosIterations; i++) {
        elevation -= abs(cnoise(vec3(modelPosition.xz * uChaosFrequency * i, uTime * uChaosSpeed)) * (uChaosElevation / i));
    }
