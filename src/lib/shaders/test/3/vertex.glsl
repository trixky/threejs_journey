// with ShaderMaterial instead of RawShaderMaterial
// we can remove projectionMatrix, viewMatrix, modelMatrix, modelViewMatrix, uv, position
...
uniform vec2 uFrequency; // CUSTOM
uniform float uTime; // CUSTOM
...
// attribute vec2 uv; // input data from javascript (uv of each vertex)
...
void main() {
    ...
    float elevationFromX = sin(modelPosition.x * uFrequency.x - uTime) * 0.05;
    float elevationFromY = sin(modelPosition.y * uFrequency.y - uTime) * 0.05;
    float elevationFromXY = elevationFromX + elevationFromY;

    modelPosition.z += elevationFromXY;

    vec4 viewPosition = viewMatrix * modelPosition;
    vec4 projectionPosition = projectionMatrix * viewPosition;

    gl_Position = projectionPosition; // CUSTOM
    vUv = uv; // CUSTOM
    vElevation = elevationFromXY; // CUSTOM
}
